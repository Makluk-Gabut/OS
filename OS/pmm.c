...
// Dibatasi 256MB dulu (lebih dari cukup buat QEMU default 128MB / hobby OS).
// Kalau nanti mau dukung RAM lebih besar, bitmap ini tinggal diperbesar.
#define MAX_SUPPORTED_MEM (256u * 1024 * 1024)
#define MAX_PAGES (MAX_SUPPORTED_MEM / PAGE_SIZE)   // 65536 halaman
#define BITMAP_SIZE (MAX_PAGES / 8)                 // 8192 byte
...
static int bitmap_test(uint32_t page) {
    if (page >= MAX_PAGES) return 1; // di luar batas dianggap "dipakai" biar aman
...
void pmm_init(struct multiboot_info* mbi) {
    // Default: semua halaman "dipakai" (aman). Baru dibebaskan kalau memory map bilang available.
    ...
    if (mbi->flags & MULTIBOOT_INFO_MEM_MAP) {
        ...
        // Fallback kalau GRUB gak kasih memory map detail: percaya mem_upper (KB, mulai dari 1MB)
    ...
    // Kalau flags dua-duanya gak ada, GRUB-nya aneh — biarin semua "dipakai", pmm_alloc_page akan selalu gagal
    // daripada diam-diam kasih memori yang belum tentu valid.

    total_pages = free_pages;

    // 1MB pertama (IVT, BIOS data, VGA buffer, dll) HARUS tetap reserved meskipun mmap bilang available,
    // karena banyak hal penting nongkrong di situ.
    for (uint32_t page = 0; page < 256; page++) reserve_page_if_free(page); // 256*4KB = 1MB

    // Area fisik kernel sendiri juga wajib reserved, biar allocator gak nimpa kode/data kernel yang lagi jalan.
...
uint32_t pmm_alloc_page(void) {
    ...
    return 0; // kehabisan memori fisik
}
